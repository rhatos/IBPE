...
const SignUpButton: React.FC = () => {
  ...
  const isModalOpen = useSelector((state: RootState) => state.registerModal.isModalOpen); // Selecting whether the registration modal is open from Redux state
  ...
  const handleSignUpClick = async () => {
    ...
    try {
      ...
      setErrMessage("Something went wrong, please contact support if this persists"); // Show a generic error message
      setIsButtonDisabled(true); // Disable the button if there's an error
    }
  };
  ...
  return (
    <>
      ...
      <Modal isOpen={isModalOpen} onClose={() => resetFormState()}> {/* Modal is controlled by the isModalOpen state */}
        ...
        <div className="space-y-6"> 
          ...
          <button
            ...
            className={`bg-bpegreen hover:bg-green-500 text-black w-28 text-center h-10 drop-shadow-xl rounded-lg flex items-center justify-center font-inter ${isButtonDisabled ? 'opacity-50 cursor-not-allowed' : ''}`} // Button styles change based on whether it's disabled
            disabled={isButtonDisabled}> {/* Button disabled based on state */}
            Sign Up
          </button>
