...
class BPEoriginal():
  ...
  def get_pair_statistics(self, vocab):
  ...
  def prune_stats(self, stats, big_stats, threshold):
  ...
  def replace_pair(self, pair, vocab, indices):
    ...
    pattern = re.compile(r'(?<!\S)' + re.escape(first + split_char + second) + r'(?!\S)')     #<---------------------------------------------THIS IS WHERE THEY ARE ADDING THE SPACE BETWEEN PAIRS
    changes = []
  ...
  def train_bpe(self, infile, vocab_size):
    found_vocabulary = []
    # Get the vocab
    vocab = self.get_vocab(infile)
    ...
    stats, indices = self.get_pair_statistics(sorted_vocab)
    ...
    for i in range(vocab_size):
      ...
      if not stats or (i and stats[most_frequent] < threshhold):
        self.prune_stats(stats, big_stats, threshhold)
      ...
      # (f"Pair {i+1}/{vocab_size}: {most_frequent[0]} {most_frequent[1]} -> {most_frequent[0]}{most_frequent[1]} [Frequency: {stats[most_frequent]}]")

      # outfile.write('{0} {1}\n'.format(*most_frequent))

      # Replace A B -> AB
      changes = self.replace_pair(most_frequent, sorted_vocab, indices)
      found_vocabulary.append(''.join(most_frequent))
      # print(f"{str(i)+": "+''.join(most_frequent)}".encode("unicode_escape").decode())

      self.update_pair_statistics(most_frequent, changes, stats, indices)
      stats[most_frequent] = 0

      # If we're not at the end prune our stats and start again.
      if not i % 100:
        self.prune_stats(stats, big_stats, threshhold)
    # print(found_vocabulary)
    return found_vocabulary

  def main(self, vocab_size, file):

    start_time = time.perf_counter()
    found_vocabulary = self.train_bpe(file, vocab_size)
